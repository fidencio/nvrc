...
/// Environment variable NVRC sets on kata-agent to select which attestation
/// agent the CoCo addon launches. With a GPU present NVRC injects the
/// [`gpu::ATTESTER_VARIANT_NVIDIA`] value so kata-agent starts the
/// NVIDIA-attester build (`attestation-agent-nv`), which emits the GPU evidence
/// the KBS attestation policy requires; absent the GPU addon the variable is
/// left unset and kata-agent keeps its stock attester.
///
/// Cross-component contract: this name and value are consumed by kata-agent
/// (`src/agent/src/main.rs`); keep them in sync.
...
/// Build the kata-agent command, injecting the attester-variant env var when an
/// `attester_variant` is given. Split out from [`exec_agent`] so the env wiring
/// is unit-testable without actually exec'ing the agent.
...
pub fn fork_agent(timeout_secs: u32) {
    ...
    match unsafe { fork() }.expect("fork agent") {
        ForkResult::Parent { .. } => {
            kata_agent(KATA_AGENT_PATH, gpu::attester_variant());
        }
        ForkResult::Child => {
...
mod tests {
    ...
    fn test_agent_command_injects_attester_variant() {
        use std::ffi::OsStr;
        let cmd = agent_command("/bin/true", Some(gpu::ATTESTER_VARIANT_NVIDIA));
        let found = cmd.get_envs().any(|(k, v)| {
            k == OsStr::new(ATTESTER_VARIANT_ENV)
                && v == Some(OsStr::new(gpu::ATTESTER_VARIANT_NVIDIA))
        });
        assert!(found, "expected {ATTESTER_VARIANT_ENV} to be set on the agent command");
    }
    ...
    fn test_agent_command_no_variant_leaves_env_unset() {
        ...
        assert!(!found, "did not expect {ATTESTER_VARIANT_ENV} without a GPU addon");
    }
