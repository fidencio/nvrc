...
impl NVRC {
    ...
    pub fn nvidia_persistenced(&mut self) {
        ...
        self.spawn_persistenced(
            ...
            &gpu::resolve("/bin/nvidia-persistenced"),
        );
    ...
    pub fn nv_hostengine(&mut self) {
        self.spawn_hostengine(&gpu::resolve("/bin/nv-hostengine"))
    }
    ...
    pub fn dcgm_exporter(&mut self) {
        self.spawn_dcgm_exporter(&gpu::resolve("/bin/dcgm-exporter"))
    }
    ...
    pub fn nv_fabricmanager(&mut self, fabric_mode: u8, rail_policy: &str) {
        // The stock config ships in the gpu addon; the editable runtime copy
        // stays on the writable /run tmpfs.
        let fm_config = gpu::resolve(FM_CONFIG);
        ...
        let mut reader = kmsg::open_kmsg("/dev/kmsg");
        self.spawn_fabricmanager(&gpu::resolve("/bin/nv-fabricmanager"));
    ...
    pub fn nv_nvlsm(&mut self) {
        self.spawn_nvlsm(&gpu::resolve("/sbin/nvlsm"))
    }
    ...
    fn spawn_nvlsm(&mut self, bin: &str) {
        ...
        let nvlsm_config = gpu::resolve(NVLSM_CONFIG);
